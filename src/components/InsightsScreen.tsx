...
const InsightsScreen = () => {
  return (
    <div className="px-4 pt-6 pb-4 space-y-6">
      ...
      <div className="tamar-card space-y-5">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Brain size={16} className="text-destructive" strokeWidth={1.5} />
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-medium text-primary">7h 30m</span>
          </div>
          <div className="w-full h-2 rounded-full bg-muted overflow-hidden">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Droplets size={16} className="text-primary" strokeWidth={1.5} />
              <span className="text-sm text-foreground">Hydration</span>
            </div>
            <span className="text-xs font-medium text-primary">2.0 L</span>
          </div>
          <div className="w-full h-2 rounded-full bg-muted overflow-hidden">
      ...
      <div className="tamar-card">
        <h3 className="text-sm font-semibold text-foreground mb-3">Stool Tracker (Bristol Scale)</h3>
        <div className="grid grid-cols-7 gap-1.5">
          {bristolTypes.map((b) => (
            ...
            >
              <span className={`text-lg font-bold ${selectedBristol === b.type ? "text-primary" : "text-muted-foreground"}`}>
                {b.type}
              </span>
              <span className="text-[7px] text-muted-foreground leading-tight mt-0.5 hidden sm:block">
                {b.desc}
              </span>
            </button>
          ))}
        ...
        <p className="text-xs text-muted-foreground mt-2">
          Today's log: <span className="font-medium text-primary">Type {selectedBristol}</span> — {bristolTypes[selectedBristol - 1].desc}
        </p>
      </div>

      {/* Safe Recipes */}
      <div>
        <h3 className="text-sm font-semibold text-foreground mb-3">Safe For You</h3>
        <div className="grid grid-cols-2 gap-3">
          {safeRecipes.map((recipe, i) => (
            <motion.div
              ...
              className="tamar-card p-0 overflow-hidden"
            >
              <img
                src={recipe.image}
                alt={recipe.name}
              ...
              <div className="p-3">
                ...
                <div className="flex items-center justify-between mt-1.5">
                  ...
                  <span className="text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-safe/15 text-safe">
                    Low Trigger
                  </span>
                </div>
              </div>
